 ...
 */
class DenyUser extends BaseCommand {
    /**
     * Class constructor
     */
    public function __construct() {
        parent::__construct('deny-user');
        $this->setDescription('Deny a user');
        $this->setHelp('Deny a user access to one or more repositories');

        $this->addArgument('user', InputArgument::REQUIRED, 'Username to deny');
        $this->addArgument('path', InputArgument::IS_ARRAY, 'Optional paths to deny the user access to in the specified repositories');

        $this->addOption('repos', null, InputOption::VALUE_OPTIONAL, 'Comma separated list of repositories to deny the user access to');
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        foreach ($repositories as $repos) {
            if (empty($paths)) {
                $rules++;
                $dbDriver->denyUser($username, $repos);
            } else {
                // Paths have been specified. Add all paths to all repositories
                foreach ($paths as $path) {
                    $rules++;
                    $dbDriver->denyUser($username, $repos, $path);
                }
            }
        }
